...
const OtpStep = ({ setStep = () => { }, email, nextStepMap }) => {
    ...
    const getOtp = async () => {
        try {
            ...
            setTimer(30);
            // Clear any lingering errors on a fresh resend
            setValidation({ isValid: true, error: "" });
        } catch (error) {
            console.error(error);
    ...
    const verifyEmail = async () => {
        ...
        try {
            ...
            if (nextStepMap[nextStep]) {
                setStep(nextStepMap[nextStep]);
            } else {
                setStep(nextStepMap["OTP"]);
            }
        }
        catch (err) {
            ...
            <div className="w-full max-w-md">
                ...
                <div className="bg-white animate-drop-custom">
                    ...
                    <div className="mt-7">
                        <OTPInput
                            ...
                            renderInput={(props) => (
                                <input
                                    {...props}
                                    className={`!w-10 !h-10 sm:!w-11 sm:!h-11 flex-none text-center text-sm border rounded-lg outline-none transition ${
                                        !validation.isValid
                    ...
                    <button
                        ...
                        className={`w-full mt-6 h-9 text-white text-sm font-medium rounded-lg flex justify-center items-center transition ${
                            verifyingEmail ?
